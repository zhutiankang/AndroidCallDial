...
import com.example.mycall0.bean.BlackNumberInfo;
import com.example.mycall0.dao.BlackNumberDAO;
import com.example.mycall0.service.CallSmsSafeService;
...
public class CallSafeActivity extends Activity {
	...
	private void fillData() {
		...
		new Thread() {
			public void run() {
				// 分页
				// infos =
				// dao.findPart(offset,maxnumber);新的数据将覆盖原来的数据属于，分页，加载数据，一页只显示20个数据
				...
				runOnUiThread(new Runnable() {
					public void run() {
						...
						// 分页，回到页面的顶端，回到原数据开始
						// adapter = new
						// MyAdapter();每次都new一个对象的话，就要重新更新一个新的界面，所以数据要重头开始
	...
	class MyAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				view = View.inflate(getApplicationContext(), R.layout.activity_list_item_call, null);
	...
	public void addBlackNumber(View view) {
		...
		View contentView = view.inflate(this, R.layout.dialog_black_number, null);
		...
		ok.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				String black_number = et_black_number.getText().toString().trim();
				...
				// 数据被添加到数据库
				dao.add(black_number);
				// 更新listview集合里面的内容,放置到最前面
				infos.add(0, new BlackNumberInfo(black_number));
				// 通知listview数据适配器数据更新了
				adapter.notifyDataSetChanged();
				System.out.println("--------------------" + infos.get(0).getNumber());
